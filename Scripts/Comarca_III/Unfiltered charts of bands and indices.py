...
bandas = sorted([x for x in convencional_bandas.columns.values if 'mean' in x], key=lambda x: band_order.get(x, 10))
...
def create_percentile_mult_chart(indices, df):
    ...
    fig.suptitle(f'Comparison of Indices for Different Management Types \n Years {year_str} in Agriculture Region {region}\n', fontsize=16)
