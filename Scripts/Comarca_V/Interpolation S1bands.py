...
bandas_s1 = pd.read_csv("S1_ASC_103_V.csv")
...
bandas_s1['REFSIGPAC']=bandas_s1['REFSIGPAC_min']
...
bandas_s1 = bandas_s1[(bandas_s1['date'] >= start_date) & (bandas_s1['date'] <= end_date)]

############################################################################################################################################

conservacion = bandas_s1[bandas_s1['Manejo'] == 'Conservacion'].copy()
convencional = bandas_s1[bandas_s1['Manejo'] == 'Convencional'].copy()
############################################################################################################################################
...
for parcela_id, df_parcela in df_por_parcela_convencional.items():
    st_parcela = df_parcela.drop_duplicates(subset=['date']).iloc[:, [i for i, col in enumerate(df_parcela.columns) if 'median' in col  or 'date' in col or'Manejo' in col or 'REFSIGPAC' in col ]]
...
df_bandas_convencional.to_csv('convencional_S1_bandas_filtrado_V.csv', index=False)

df_bandas_conservacion.to_csv('conservacion_S1_bandas_filtrado_V.csv', index=False)
