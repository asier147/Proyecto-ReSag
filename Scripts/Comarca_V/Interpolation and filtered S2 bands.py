# -*- coding: utf-8 -*-
"""
Created on Sun May 19 12:07:56 2024
...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 13 17:41:11 2024

@author: Asier


Interpolar las ST con huecos generadas y suavizar las curvas mediante el filtro de SG 
1) Filtrar outliers
2) Interpolar
3) Aplicar el filtro SG
...
convencional_bandas = pd.read_csv("Comarca_V_median_bandas_S2_convencional.csv")
convencional_bandas.rename(columns={'PRD_DESC': 'Cultivo'}, inplace=True)
conservacion_bandas = pd.read_csv("Comarca_V_median_bandas_S2_conservacion.csv")
...
for parcela_id, df_parcela in df_por_parcela_convencional.items():
    ...
    for col in df_parcela.columns:
        if col not in['date','Manejo','REFSIGPAC','Cultivo']:  # Excluir la columna 'date'
...
def int_lineal(st_parcela):
    ...
    for column in st_parcela.columns:
        if col not in['Manejo','REFSIGPAC','Cultivo']:
...
for parcela_id, df_parcela in sin_outliers_por_parcela_convencional.items():
    st_parcela = df_parcela.drop_duplicates(subset=['date']).iloc[:, [i for i, col in enumerate(df_parcela.columns) if 'median' in col  or 'date' in col or'Manejo' in col or 'REFSIGPAC' in col or 'Cultivo' in col]]
...
for parcela_id, df_parcela in sin_outliers__interpolacion_por_parcela_convencional.items():
    st_parcela_int=df_parcela.iloc[:, [i for i, col in enumerate(df_parcela.columns) if 'median' in col or 'date' or 'Manejo' in col or 'REFSIGPAC' in col or 'Cultivo' in col]]
...
guardado_path = "C:/Users/Asier/Desktop/Proyecto ReSAg/Archivos/Comarca_V"
os.chdir(guardado_path)
df_bandas_convencional.to_csv('convencional_V_bandas_filtrado.csv', index=False)

df_bandas_conservacion.to_csv('conservacion_V_bandas_filtrado.csv', index=False)
