...
def _arrows_(ax, sup_90, sup_80, C, maxval, dist_arrow = 300):
    ...
    txt = '{}% samples'.format(round(sup_90/len(C)*100))
    ax.annotate(text=txt, xy=(1,5100), xytext=(0.92,5100))
    txt = '{}% samples'.format(round(sup_80/len(C)*100))
...
for i,archivo in enumerate(archivos):
    ...
    ax.set_ylabel('Model 7\nNo. samples',fontsize=14)
    _arrows_(ax, sup_90_ca, sup_80_ca, CA, max_val)
    ax.set_title('Conservation',fontsize=16,pad=20)
    ax.set_xlabel('Probability', fontsize=14)
    #Convencional
    ax = axs[1]
    # ax = axs_g[i,1]
    sns.histplot(CT[colint], bins = bins, ax = ax)
    ax.set_title('Conventional',fontsize=16,pad=20)
